...
description: Minimum overviews of model-based RL methods
date: 2023-11-01
...
Maybe this is out-of-place here:

## Recent advances in model-based RL

The PETS paper was the first to address these challenges in the field. By addressing uncertainties in learned models using probabilistic models and ensembles, PETS managed to match the asymptotic performance of model-free methods in challenging domains, all while requiring fewer samples. Though PETS achieved this in 2018, recent advancements have further refined model-based RL approaches.

Mention Danijer Haffner’s works, Cansu, FOCUS 
...
2. Where would I be if I turned right or left here?

Holland writes: “ an internal model allows a system to look ahead to the future consequences of current actions, ****************without**************** actually committing itself to those actions” (Holland, 1992, p25).
...
In simple mathematical terms, world models learns a function F: S_t x A→ S_t+1 or predicting the next state given the observed state(s) and current action.

# General Algorithm

This picture illustrates the general paradigm of model-based RL:

![mbrl_general.jpeg](https://prod-files-secure.s3.us-west-2.amazonaws.com/6cc79589-dd02-4086-b690-2bf30a0f3a2e/58fb6926-b01b-492e-8bfa-95e86a898897/mbrl_general.jpeg)
...
x_t+1 is conditionally independent of x_{1:t-1} given x_t, a_t
...
This started out in 60s with linear models, now became popular with RL.
